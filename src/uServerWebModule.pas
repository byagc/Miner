...
uses uServerMethods, uServerContainer, Web.WebReq, uServerMain;
...
var
  Aux: TJSONValue;
begin
  frmMain.Memo1.Lines.Add('WebDispatcher1FormatResult: ' + Command.Text);

  if Command.Text = 'DSAdmin.Search' then
...
begin
  if SameText(TagString, 'urlpath') then
    ...
    ReplaceText := uServerMethods.TServerMethods.ClassName
  else if SameText(TagString, 'loginrequired') then
    if ( DSHTTPWebDispatcher1.AuthenticationManager <> nil) then //   or  ( frmMain.CheckBox1.Checked )
      ReplaceText := 'true'
    else
      ReplaceText := 'false'
...
begin
  ...
  DSHTTPWebDispatcher1.Server := DSServer;  // DSServer1
